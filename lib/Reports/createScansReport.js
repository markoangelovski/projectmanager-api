...
const createScansReport = scans => {
  ...
  scans.forEach(scan => {
    ...
    if (scan.stats.length) {
      scan.stats.forEach(stat => {
        ...
        // Check which error has the most counts and create a row for each missing GTM key or a difference in GTM key/attribute values
  ...
  // Report Filename
  const reportFile = `reports/ScansReport-${moment().format("YYYY-MM-DD")}.csv`;
  ...
  return `ScansReport-${moment().format("YYYY-MM-DD")}.csv`;
};
